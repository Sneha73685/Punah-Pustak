...
- **DEPLOY-022**: Alembic migrations MUST run as an explicit release step (not on application boot), so a failed migration doesn't leave multiple app instances racing to migrate concurrently. If a migration fails during this step, the deployment MUST halt automatically and MUST NOT proceed to deploy new application code; recovery requires manual engineering review before retry — automatic rollback tooling is not required at this scale *(clarified in 2.1.0)*.
