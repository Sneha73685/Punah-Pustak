...
"Client IP" is not simply `request.client.host` in production: this service is deployed on Render behind Render's own edge/load-balancer layer and, per Render's documented platform architecture, Cloudflare in front of that — the raw TCP peer uvicorn sees is one of those hops, not the real client. `TRUSTED_PROXY_HOP_COUNT` (`Settings.trusted_proxy_hop_count`) controls how many such hops are trusted to resolve the real client from `X-Forwarded-For`: 0 (local dev, CI — no reverse proxy sits in front of uvicorn there) means "trust nothing, use the raw TCP peer"; production sets it to 2. The extraction (`app.modules.auth.dependencies._extract_client_ip`) never trusts the header's left-most entry, since neither Render's edge nor Cloudflare strips a client-supplied `X-Forwarded-For` value before appending to it — a client could otherwise forge their own rate-limit bucket. Instead it takes exactly the entry `trusted_proxy_hop_count` positions from the right (the "N trusted hops" algorithm), falling back to the raw TCP peer if the header doesn't carry enough entries for the configured count.
