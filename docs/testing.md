...
## End-to-end: accessibility (axe-core)

SRS §A11Y-007's automated WCAG 2.1 AA regression gate — Milestone 6 scope — is [`e2e/a11y.spec.ts`](../e2e/a11y.spec.ts), a fourth spec in the same Playwright suite above, using the identical Docker Compose stack and `npm test` command (no separate tool, config, or CI job). It runs [`@axe-core/playwright`](https://www.npmjs.com/package/@axe-core/playwright) against the real rendered DOM (never source markup in isolation), scoped to the `wcag2a`/`wcag2aa`/`wcag21a`/`wcag21aa` rule tags via the shared [`e2e/helpers/a11y.ts`](../e2e/helpers/a11y.ts) helper — `best-practice` rules are deliberately excluded, since those flag opinionated authoring conventions with no WCAG success criterion behind them, and a required CI gate should not fail builds over things that aren't the actual conformance target.

Covers the full A11Y-007 gated page set (browse, listing detail, create-listing, login, registration, and `/change-password` — reached the same way production reaches it, via a real admin-triggered reset followed by the forced-change redirect, not a direct `goto`) plus every other FE-002 route (home, my listings, profile, both admin pages), and the UI states most likely to differ accessibility-wise from a bare happy path: empty results, populated grids/tables, client- and server-side validation errors, a confirmation modal open (`Modal`'s `role="dialog"`/focus-trap machinery, A11Y-004), authenticated nav, and the mobile nav panel at a phone viewport width (the project's single-Chromium-project scope, per `playwright.config.ts`, is preserved — this uses `page.setViewportSize` rather than adding a second browser/device project).

**Current state:** running against this codebase's actual component library (`Input`/`Select`'s `htmlFor`/`aria-describedby` pattern, `Modal`'s manual focus trap, the `index.css` contrast fixes already made for `ink-soft`/`gold-600`), the suite finds **zero violations** across every page/state above — 23 WCAG A/AA rules evaluated per page, including the rendering-dependent `color-contrast` check, confirmed by inspecting axe's raw output directly rather than trusting a passing assertion alone. Nothing needed fixing to make this gate pass; it exists from here on to catch regressions.
