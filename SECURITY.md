...
- **A genuine same-instant multi-tab refresh race can still force a full re-login.** `/auth/refresh` is now safe against the data-corruption case (concurrent presentations of one token resolve to exactly one winner, not several), but the losing request's reuse-detection path still revokes the whole token family, including the winner's brand-new token — see [`docs/authentication.md`](docs/authentication.md#refresh-rotation-and-reuse-detection). This is a UX/availability trade-off (an unwanted logout), not an authorization bypass or a way to obtain a session that shouldn't exist.
